"""piqe - Utility to calculate image quality without reference.
...
def noticeDistCriterion(
    Block, nSegments, blockSize, windowSize, blockImpairedThreshold, N
):
    # Top edge of block
    topEdge = Block[0, :]
...
def piqe(im: np.ndarray, block_size: int = None):
    ...
    activityThreshold = (
        0.1  # Threshold used to identify high spatially prominent blocks
    )
    blockImpairedThreshold = (
