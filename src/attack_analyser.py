...
class AttackAnalyser:
    ...
    def _compile_attack_results(
        ...
        self.create_combined_averaged_roc_curve(attack_type, attack_stores,
                                                run_number)
        self.create_combined_attack_metric_bar_chart(attack_type,
                                                     attack_stores, run_number)
    ...
    def create_combined_attack_metric_bar_chart(
        ...
        for store in attack_store:
            ...
            num_classes = store.attack_result_df[
                store.attack_result_df["slice value"].fillna(
                    value=" ").str.isnumeric()]["slice value"].max()
            num_classes = int(num_classes) + 1

            for class_i in range(num_classes):
                class_values = store.attack_result_df.loc[
                    f"mean CLASS={class_i}"]
        ...
        fig_fpr01, ax_fpr01 = plt.subplots(figsize=FIGSIZE,
                                           layout="constrained")
        fig_fpr0001, ax_fpr0001 = plt.subplots(figsize=FIGSIZE,
                                               layout="constrained")
        ...
        print(
            f"Saving class wise bar chart FPR@0.1 figure to {fpr01_bar_chart_fn}"
        )
        ...
        print(
            f"Saving class wise bar chart FPR@0.001 figure to {fpr0001_bar_chart_fn}"
        )
