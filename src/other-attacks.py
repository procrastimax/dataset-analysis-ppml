...
class MiaAttack:
    ...
    def initialize_data(self):
        ...
        self.loss_train = self.scce(
            self.constant(self.train_labels),
            self.constant(self.prob_train),
            from_logits=False,
        ).numpy()
        self.loss_test = self.scce(
            self.constant(self.test_labels),
            self.constant(self.prob_test),
            from_logits=False,
        ).numpy()
    ...
    def run_mia_attack(self):
        ...
        if self.input is None:
            print(
                "Error: Please run 'initialize_data()' before! Uninitialized AttackInputData!"
            )
            sys.exit(1)
        ...
        attacks_result = mia.run_attacks(
            self.input, attack_types=attack_types, slicing_spec=slicing_spec
        )
...
class MembershipProbability:
    ...
    def calc_membership_probability(
        self, plot_training_samples: bool = True, num_images: int = 5
    ):
    ...
    def _plot_training_samples(self, num_images: int = 5):
        for c in range(self.ds_classes):
            ...
            class_train_membership_probs = mia._compute_membership_probability(
                class_input_slice
            ).train_membership_probs

            class_high_risk_idx = np.argsort(class_train_membership_probs)[::-1][
                :num_images
            ]
            class_low_risk_idx = np.argsort(
                np.absolute(class_train_membership_probs - 0.5)
            )[:num_images]
