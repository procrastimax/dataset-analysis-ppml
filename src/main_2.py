from ppml_datasets import Cifar10Dataset, MnistDataset, FashionMnistDataset
...
def main():
    ...
    # fmnist = FashionMnistDataset([32, 32, 3], builds_ds_info=False, batch_size=batch, augment_train=False)
    # fmnist.load_dataset()
    # fmnist.prepare_datasets()

    cifarx = Cifar10Dataset([32, 32, 3], builds_ds_info=False, batch_size=batch, augment_train=False)
    cifarx.load_dataset(lambda x, y: filter_labels(y, classes))
...
def run_amia_attack(ds: AbstractDataset, model: CNNModel):
    ...
    shadow_model_save_path: str = os.path.join(data_path, model_path, "shadow_models", f"cifar{num_classes}", str(run_number))

    amia = AmiaAttack(model=model, dataset=ds, num_classes=num_classes,
                      num_shadow_models=16, shadow_model_dir=shadow_model_save_path,
