...
from util import pickle_object, unpickle_object, find_nearest, save_dataframe
...
class Analyser():
    ...
    def generate_results(self):
        for (ds_name, ds_store) in self.dataset_data.items():
            ...
            ds_store.attack_result_df = ds_store.calculate_tpr_at_fixed_fpr(ds_store.attack_result_list, attack_name="amia")
            ds_store.attack_baseline_result_df = ds_store.calculate_tpr_at_fixed_fpr(ds_store.attack_baseline_result_list, attack_name="mia")
            ...
            ds_store.create_average_roc_curve(attack_result_list=ds_store.attack_result_list, generate_all_rocs=True, generate_std_area=True)
            # ds_store.create_average_roc_curve(attack_result_list=ds_store.attack_result_list, enerate_all_rocs=True, generate_std_area=False)
            # ds_store.create_average_roc_curve(attack_result_list=ds_store.attack_result_list, enerate_all_rocs=False, generate_std_area=True)
            # ds_store.create_average_roc_curve(attack_result_list=ds_store.attack_result_list, enerate_all_rocs=False, generate_std_area=False)
