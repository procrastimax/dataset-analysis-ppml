...
class CNNModel():
    def __init__(self, img_height: int, img_width: int, color_channels: int, num_classes: int, conv_layer_filter_dim_list: List[int] = [16, 32, 64], conv_layer_kernel_dim_list: List[int] = [3, 3, 3], dense_layer_dimension: int = 128, optimizer="adam"):
    ...
    def build_model(self):
        ...
        for (i, (filter_dim, kernel_dim)) in enumerate(zip(self.filter_dim_list, self.kernel_dim_list)):
    ...
    def compile_model(self):
        self.model.compile(optimizer=self.optimizer, loss=tf.keras.losses.SparseCategoricalCrossentropy(
    ...
    def train_model(self, train_ds: tf.data.Dataset, val_ds: tf.data.Dataset, epochs: int):
